...
if __name__ == "__main__":
    ...
    tags = ["[5FWD,HLT,CST]", "[HLT,5FWD,CST]", "[ORG,HLT,CST]", "[CST,ORG,HLT]", "[HLT,ORG,CST]"]
    ...
    for i in range(0, len(tags)):
        ...
        for j in range(0, len(hidden)):
            ...
            params = TrainingParameters(num_episodes=50, hidden_size=hidden[j], update_every=4, batch_size=batch[j], reward_size = 3,
                                    sample_size = 20, bias = True, nohid = False, slack = 0.01, loss_threshold = 0.5, env_name = "Ant-v4", 
                                    agent_name = "LexCDQN", num_test = 10)
    ...
    tags = ["[5FWD] [HLT] [CST]", "[ORG] [HLT]", "[ORG] [CST]", "[ORG] [HLT] [CST]", '[ORG] [10HLT]', '[ORG]']
    descriptor_base = "Ant [EP] [AWG] [EPS] [20S] | "
    process_id = int(str(time.time())[-5:])
    ...
    for i in range(0, len(tags)):
        ...
        for j in range(0, len(hidden)):
            ...
            params = TrainingParameters(num_episodes=10, hidden_size=hidden[j], update_every=4, batch_size=batch[j], 
                                    sample_size = 20, bias = True, nohid = False, env_name = "Ant-v4", 
                                    agent_name = "ContinuousDQN", num_test = 10)
            session_pref = os.path.join("./test", descriptor, datetime.now().strftime('%Y%m%d-%H%M%S'))
